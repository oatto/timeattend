...
import { NOTIFICATION_TIME_ADJUSTMENT_TYPE, STATE_REJECTED_AND_CANCELLED } from '_features/common/redux/constants';
...
import { fetchMngCheckTimeAdjustmentRejected } from '_features/mng-check-time-adjustment/redux/actions';
import { mngCheckTimeAdjustmentRejected } from '_features/mng-check-time-adjustment/redux/selectors';
...
class _rejected extends React.PureComponent {
    ...
    componentDidMount() {
        this.props.fetchMngCheckTimeAdjustmentRejected.request({transition: STATE_REJECTED_AND_CANCELLED});
    ...
    _onRefresh() {
        this.props.fetchMngCheckTimeAdjustmentRejected.refresh({
    ...
    render() {
        const { data, isRefreshing, isLoadingMore, isLoading, pagination } = this.props.mngCheckTimeAdjustmentRejected;
                        ...
                        <Body>
                            <EmployeeSearchForm2
                                onSubmit={(values) => {
                                    ...
                                    this.props.fetchMngCheckTimeAdjustmentRejected.request(values);
                                }}
                            />
                        </Body>
                    </CardItem>
                    ...
                    data.length ?
                        <InfinityScrollList
                            ...
                            onLoadMore={() => {
                                ...
                                this.props.fetchMngCheckTimeAdjustmentRejected.loadmore(this.state.searchValues);
                            }}
...
export default connect(
    (state) => ({
        mngCheckTimeAdjustmentRejected: mngCheckTimeAdjustmentRejected(state),
        notificationCenterListSelector: notificationCenterListSelector(state)
    }),
    (dispatch) => ({
        navigation: bindActionCreators(NavigationActions, dispatch),
        fetchMngCheckTimeAdjustmentRejected: bindActionCreators(fetchMngCheckTimeAdjustmentRejected, dispatch),
