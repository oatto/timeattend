...
import {
    ...
    getLocationByToken,
    getLocationByTokenAndDeviceLatLong
} from '../actions'
import {
    CREATE_CHECK_IN,
    CREATE_CHECK_OUT,
...
export const watchSelectLocaionByQrCodeAndDeviceLatLong = function*() {
    yield takeLatest('SELECT_LOCATION_BY_QR_CODE_AND_DEVICE_LAT_LONG', function*({payload}) {
        yield put(getLocationByTokenAndDeviceLatLong.request({
            token: payload.token,
            deviceLatitude: payload.forwardedParams.coordinate.latitude,
            deviceLongitude: payload.forwardedParams.coordinate.longitude
        }));
        ...
        const action = yield take([
            GET_LOCATION_BY_TOKEN_AND_DEVICE_LAT_LONG.SUCCESS,
            GET_LOCATION_BY_TOKEN_AND_DEVICE_LAT_LONG.FAILURE,
        ]);

        if (action.type === GET_LOCATION_BY_TOKEN_AND_DEVICE_LAT_LONG.SUCCESS) {
            yield put(NavigationActions.navigate({ routeName: CHECK_TIME, params: {
                checkType: {
                    typeName: payload.forwardedParams.typeName,
                    typeValue: payload.forwardedParams.typeValue,
        ...
        if (action.type === GET_LOCATION_BY_TOKEN_AND_DEVICE_LAT_LONG.FAILURE) {
            Alert.alert(
                Trans.tran('general.notice'),
                action.errors.response.data.message, [
...
export const watchGetLocationByTokenAndDeviceLatLongRequest = function*() {
    yield takeLatest(GET_LOCATION_BY_TOKEN_AND_DEVICE_LAT_LONG.REQUEST, function*(action) {
        // do staff
        yield call(doRequest, getLocationByTokenAndDeviceLatLong, {
            apiFunction: LocationApi.getLocationWithTokenAndDeviceLatLong,
            args: [action.payload]
        })
    });
};
